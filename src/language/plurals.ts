...
export const PLURALIZATION_RULES = {
    ...
    LanguageCode,
    {
        cardinal: PluralOption[];
        ordinal: PluralOption[];
    }
>;
...
type PluralizationFunctionCardinal = Record<LanguageCode, unknown> & {
    [key in LanguageCode]: (
        value: number,
    ) => (typeof PLURALIZATION_RULES)[key]["cardinal"][number];
};
...
export const PLURALIZATION_FUNCTIONS_CARDINAL: PluralizationFunctionCardinal = {
    [LanguageCode.de_DE]: (value: number) =>
        value === 1 ? PluralOption.One : PluralOption.Other,
    ...
    [LanguageCode.es_ES]: (value: number) => {
        ...
        const compactDecimalExponentValue =
    ...
    [LanguageCode.ru_RU]: (value: number) => {
        const hasDecimal = value % 1 !== 0;
        const int = math.floor(value);
        const int10 = int % 10;
        const int100 = int % 100;
        if (!hasDecimal && int10 === 1 && int100 !== 11)
            return PluralOption.One;
        if (
            !hasDecimal &&
            int10 >= 2 &&
            int10 <= 4 &&
    ...
    [LanguageCode.tr_TR]: (value: number) =>
        value === 1 ? PluralOption.One : PluralOption.Other,
...
type PluralizationFunctionOrdinal = Record<LanguageCode, unknown> & {
    ...
    [key in LanguageCode]: (
        value: number,
    ) => (typeof PLURALIZATION_RULES)[key]["ordinal"][number];
};
...
export const PLURALIZATION_FUNCTIONS_ORDINAL: PluralizationFunctionOrdinal = {
    ...
    [LanguageCode.ru_RU]: (_value: number) => PluralOption.Other,
    [LanguageCode.th_TH]: (_value: number) => PluralOption.Other,
    [LanguageCode.tr_TR]: (_value: number) => PluralOption.Other,
    [LanguageCode.vi_VN]: (value: number) =>
        value === 1 ? PluralOption.One : PluralOption.Other,
    [LanguageCode.zh_CN]: (_value: number) => PluralOption.Other,
    [LanguageCode.zh_TW]: (_value: number) => PluralOption.Other,
};
